...
public class PickUp extends JFrame {
	...
	public PickUp() throws SQLException {
		...
		lblPickUpService.setFont(new Font("Algerian", Font.BOLD, 35));
		lblPickUpService.setBounds(250, 18, 500, 50);
		...
		contentPane.add(lblTypeOfCar);
                ...
                try{
                    conn c = new conn();
                    ResultSet rs = c.s.executeQuery("select * from divers");
                    while(rs.next()){
		...
		btnRegister.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String SQL = "select * from divers where brand = '"+c1.getSelectedItem()+"'";
				try{
